...
class MapMarker
{
	constructor(name, icon, cordX, cordY, map)
	{
		this.map = map;
		this.name = name;
		this.icon = icon;
		...
		this.generateMarker = function () {
			var output = '<div class="KBmap__marker" data-marker-name="' + this.name + '" style="left: ' + this.cordX + '%; top: ' + this.cordY + '%"><img src="' + this.icon + '"></div>';
			//output = '<div class="KBmap__marker" data-marker-name="'+this.name+'" style="left: '+this.cordX+'%; top: '+this.cordY+'%">'+this.icon+'</div>'
			return output;
		};
...
function MarkerModal(modalTitle, content, linkedMapMarker){
	...
	this.generateModal = function(){
		output = '<div  class="KBmap__markerContent"><div class="KBmap__markerClose"><i class="fa fa-times" aria-hidden="true"></i></div><h5 class="KBmap__markerTitle">' + this.title + '</h5>';
