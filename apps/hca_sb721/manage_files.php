...
$access = ($User->checkAccess('hca_sb721', 13)) ? true : false;
if (!$access)
...
$query = array(
	'SELECT'	=> 'pj.*, pt.pro_name, pt.manager_email, u.realname',
...
if (isset($_POST['upload_file']))
{
    ...
    $SwiftUploader->uploadFiles('hca_sb721_projects', $id);
...
else if (isset($_POST['send_files']))
{
	...
	if (empty($Core->errors))
	{
		...
		$url_param = [];
		$url_param[] = 'project=hca_sb721_projects';
		...
		//$SwiftMailer->isHTML();
		$SwiftMailer->send($emails, 'SB-721 Project', $mail_message);
...
$query = array(
	...
	'FROM'		=> 'sm_uploader',
	'WHERE'		=> 'table_name=\'hca_sb721_projects\' AND table_id='.$id,
);
$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
...
$Core->set_page_id('hca_sb721_manage_files', 'hca_sb721');
...
<?php
$mail_message = [];
$mail_message[] = 'This is the SB-721 Project.'."\n";
...
{
	$('form input[name="upload_file"]').css("pointer-events","none");
	$('form input[name="upload_file"]').val("Processing...");
}
</script>
