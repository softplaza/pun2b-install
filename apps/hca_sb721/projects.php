...
$access = ($User->checkAccess('hca_sb721', 1)) ? true : false;
$permission_level2 = ($User->checkAccess('hca_sb721', 12)) ? true : false;
if (!$access)
	message($lang_common['No permission']);

$HcaSB721 = new HcaSB721;
...
if ($section == 'completed')
	$search_query[] = 'pj.project_status=5';
else if ($section == 'removed')
	$search_query[] = 'pj.project_status=0';
else
	$search_query[] = 'pj.project_status=1';
...
else if (isset($_POST['complete_project']))
{
	...
	if ($id > 0)
	{
		$DBLayer->update('hca_sb721_projects', ['project_status' => 5], $id);

		$flash_message = 'Project completed';
		$FlashMessenger->add_info($flash_message);
		redirect($URL->link('hca_sb721_projects', ['completed', $id]), $flash_message);
	}
}
...
else if (isset($_POST['remove_project']))
{
	...
	if ($id > 0)
	{
		$DBLayer->update('hca_sb721_projects', ['project_status' => 0], $id);
...
if (!empty($main_info))
{
	$query = array(
		...
		'ORDER BY'	=> 'v2.date_bid, v2.date_start_job',
		...
		<table class="table table-sm table-striped table-bordered">
			<thead class="sticky-under-menu">
				<tr class="ta-center">
					...
					<th class="min-100">Project ID#</th>
					<th class="min-100">Pre-Inspection Date</th>
					<th>Symptoms</th>
					<th>Action</th>
					<th class="min-100">Engineer Inspection Date</th>
...
<?php
	foreach ($main_info as $cur_info)
	{
		...
		if ($permission_level2)
		{
			...
			if ($cur_info['project_status'] == 1)
				$Core->add_dropdown_item('<button name="complete_project['.$cur_info['id'].']" class="badge bg-success">Complete project</button>');
			else
				$Core->add_dropdown_item('<button name="active_project['.$cur_info['id'].']" class="badge bg-primary">Make active</button>');

			if ($cur_info['project_status'] != 0)
				$Core->add_dropdown_item('<button name="remove_project['.$cur_info['id'].']" class="badge bg-danger" onclick="return confirm(\'Are you sure you want to delete this project?\')">Remove project</button>');
		...
		$page_param['td']['locations'] = ($cur_info['locations'] != '') ? '<p class="text-primary">'.html_encode($cur_info['locations']).'</p>' : '';
		...
		foreach ($follow_up_info as $cur_follow_up)
		{
			...
			{
				$status = (format_date($cur_follow_up['date_time'], 'Y-m-d') == date('Y-m-d')) ? 'alert-danger' : 'alert-info';
				$follow_up_dates[] = '<div class="'.$status.' mb-1 p-1">';
				if ($permission_level2)
		...
		if (!empty($sb721_vendors))
		{
			foreach($sb721_vendors as $cur_vendor)
			{
				...
				{
					if (format_date($cur_vendor['date_bid'], 'Y-m-d') == date('Y-m-d') || format_date($cur_vendor['date_start_job'], 'Y-m-d') == date('Y-m-d') || format_date($cur_vendor['date_end_job'], 'Y-m-d') == date('Y-m-d'))
						$vendor_status = 'alert-danger';
					else if (format_date($cur_vendor['date_end_job']) != '')
						$vendor_status = 'alert-primary';
					else if (format_date($cur_vendor['date_start_job']) != '')
						$vendor_status = 'alert-success';
					else if (format_date($cur_vendor['date_bid']) != '')
						$vendor_status = 'alert-warning';
					else
						$vendor_status = 'alert-grey';

					$cur_vendors[] = '<div class="'.$vendor_status.' mb-1 p-1">';
					...
					if (format_date($cur_vendor['date_bid']) != '')
						$cur_vendors[] = '<p>Bid: '.format_date($cur_vendor['date_bid'], 'n/j/y').'</p>';
					if (format_date($cur_vendor['date_start_job']) != '')
						$cur_vendors[] = '<p>Start job: '.format_date($cur_vendor['date_start_job'], 'n/j/y').'</p>';
					if (format_date($cur_vendor['date_end_job']) != '')
						$cur_vendors[] = '<p>End job: '.format_date($cur_vendor['date_end_job'], 'n/j/y').'</p>';
...
?>
				<tr id="row<?php echo $cur_info['id'] ?>" class="<?php echo ($cur_info['id'] == $pid) ? 'anchor' : '' ?>">
					...
					<td class="ta-center"><span class="fw-bold"><?php echo html_encode($cur_info['project_number']) ?></span></td>
					<td>
