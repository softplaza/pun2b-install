...
if (isset($_POST['upload_file']))
{
    ...
    $SwiftUploader->uploadFiles('hca_hvac_inspections_checklist', $id);
...
else if (isset($_POST['send_files']))
{
	...
	if (empty($Core->errors))
	{
		...
		$url_param = [];
		$url_param[] = 'project=hca_hvac_inspections_checklist';
...
$query = array(
	...
	'FROM'		=> 'sm_uploader',
	'WHERE'		=> 'table_name=\'hca_hvac_inspections_checklist\' AND table_id='.$id,
);
$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
$sm_uploader = $files_info = $media_info = array();
while ($row = $DBLayer->fetch_assoc($result))
{
	$sm_uploader[] = $row;
}

$Core->set_page_id('hca_hvac_inspections_files', 'hca_hvac_inspections');
...
<?php endif; ?>
	...
	<form method="post" accept-charset="utf-8" action="">
		<input type="hidden" name="csrf_token" value="<?php echo generate_form_token() ?>" />
<?php
	$SwiftUploader->getProjectFiles('hca_hvac_inspections_checklist', $id);
?>
		<div class="card">
			<div class="card-header">
			...
			<div class="card-body">
<?php
if (!empty($sm_uploader))
{
?>
				<div class="uploaded-images">
<?php

	foreach($sm_uploader as $cur_file)
	{
		$cur_link = BASE_URL.'/'.$cur_file['file_path'].$cur_file['file_name'];
		$preview = ($cur_file['file_type'] == 'image') ? '<a data-fancybox="single" href="'.$cur_link.'" target="_blank"><img src="'.$cur_link.'"></a>' : '<a href="'.$cur_link.'" target="_blank"><i class="far fa-file-alt fa-8x text-primary"></i></a>';
