...
if ($action == 'pdf')
{
	//$HcaHVACInspectionsPDF = new HcaHVACInspectionsPDF;
	//$HcaHVACInspectionsPDF->genWorkOrder();
?>
...
foreach($HcaHVACInspections->locations as $location_id => $location_name)
{
	if (!empty($checked_items))
	{
		...
		foreach($checked_items as $cur_info)
		{
			...
			{
				$element_name = $HcaHVACInspections->getEquipment($cur_info['equipment_id']).' -> '.html_encode($cur_info['item_name']);
				$problem_names = $HcaHVACInspections->getItemProblems($cur_info['problem_ids']);
