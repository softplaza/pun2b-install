...
$HcaHVACAppendixB = new HcaHVACAppendixB;
...
if (isset($_POST['create_pdf']) && isset($_POST['form']))
{
	...
	$html_content = $HcaHVACAppendixB->gen_appendix_b($form_info, $project_info);
	
	$mpdf = new \Mpdf\Mpdf();
	$mpdf->WriteHTML($html_content);
	
	$time_now = time();
	$assoc_table_name = 'hca_hvac_inspections_checklist';
	...
	$file_path = $SwiftUploader->checkPath('hca_hvac_inspections_checklist').'/';
	...
	if (file_exists($full_file_path))
	{
		...
		$emails = $User->getNotifyEmails('hca_hvac_inspections', 1); // 1 - appendix-b created
		...
		redirect($URL->link('hca_hvac_inspections_files', $id), $flash_message);
	}
	else
...
$Core->set_page_id('hca_hvac_inspections_appendixb', 'hca_hvac_inspections');
			...
			<div class="pdf-content">
				...
				<table>
					<tbody>
						...
						<tr>
							<td><span class="box-title">Date Reported:&nbsp;&nbsp;<input type="datetime-local" name="reported_time" value="" required></span></td>
							<td class="td2-4"></td>
...
<?php endif; ?>
			...
			<div class="row mb-1">
				<div class="col-md-6">Location - which room / room(s) etc.</div>
<?php
$locations = $HcaHVACInspections->locations;
foreach($locations as $location_id => $location_name)
{
	echo '<div class="col-md-2"><input type="text" name="form[location'.$location_id.']" value="'.html_encode($location_name).'" required></div>'."\n";
}
?>
			</div>
			<div class="row mb-1">
				<div class="col-md-6">Square Footages</div>
<?php
foreach($locations as $location_id => $location_name)
{
	echo '<div class="col-md-2"><input type="text" name="form[square_footages'.$location_id.']" value="" required></div>'."\n";
}
?>
			</div>
			<div class="row mb-1">
				<div class="col-md-6">Wood moisture meter results</div>
<?php
foreach($locations as $location_id => $location_name)
{
	echo '<div class="col-md-2"><input type="text" name="form[wood_results'.$location_id.']" value="" required></div>'."\n";
}
?>
			</div>
			<div class="row mb-1">
				<div class="col-md-6">Concrete moisture meter results</div>
<?php
foreach($locations as $location_id => $location_name)
{
	echo '<div class="col-md-2"><input type="text" name="form[concrete_results'.$location_id.']" value="" required></div>'."\n";
}
?>
			</div>
