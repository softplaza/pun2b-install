...
$access = ($User->checkAccess('hca_vcr', 4)) ? true : false;
if (!$access)
...
if (isset($_POST['upload_file']))
{
    ...
    $SwiftUploader->uploadFiles('hca_vcr_projects', $id);
...
else if (isset($_POST['send_files']))
{
	...
	if (empty($Core->errors))
	{
		...
		$url_param = [];
		$url_param[] = 'project=hca_vcr_projects';
...
$query = array(
	...
	'FROM'		=> 'sm_uploader',
	'WHERE'		=> 'table_name=\'hca_vcr_projects\' AND table_id='.$id,
);
$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
...
$Core->set_page_id('hca_vcr_manage_files', 'hca_vcr');
...
<?php endif; ?>
	...
	<form method="post" accept-charset="utf-8" action="">
		<input type="hidden" name="csrf_token" value="<?php echo generate_form_token() ?>" />
<?php
	$SwiftUploader->getProjectFiles('hca_vcr_projects', $id);
?>
		<div class="card">
			<div class="card-header">
...
$mail_message = [];
//$mail_message[] = 'Hello,';
$mail_message[] = 'This is the Vacancy Condition Report.'."\n";
