...
if ($start_date > 0)
{
	...
	if (!empty($maint_list))
	{
		$json_array['maint_list'] = '<select name="maint_id" onchange="getMaintTimeSlot()">'."\n";
		$json_array['maint_list'] .= '<option value="0">Select Maintenance</option>'."\n";
		foreach($maint_list as $cur_info)
		{
			...
			if (in_array(1, $busy_slots) || (in_array(2, $busy_slots) && in_array(3, $busy_slots)))
				$json_array['maint_list'] .= '<option value="'.$cur_info['id'].'" disabled style="color:red">'.$cur_info['realname'].'</option>'."\n";
	...
	echo json_encode(array(
		'maint_list'		=> $json_array['maint_list'],
	));	
}
