...
if (isset($_POST['upload_file']))
{
    ...
    $SwiftUploader->uploadFiles('hca_unit_inspections_checklist', $id);
...
else if (isset($_POST['send_files']))
{
	...
	if (empty($Core->errors))
	{
		...
		$url_param = [];
		$url_param[] = 'project=hca_unit_inspections_checklist';
...
$query = array(
	...
	'FROM'		=> 'sm_uploader',
	'WHERE'		=> 'table_name=\'hca_unit_inspections_checklist\' AND table_id='.$id,
);
$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
...
$Core->set_page_id('hca_unit_inspections_files', 'hca_unit_inspections');
...
<?php endif; ?>
	...
	<form method="post" accept-charset="utf-8" action="">
		<input type="hidden" name="csrf_token" value="<?php echo generate_form_token() ?>" />
<?php
	$SwiftUploader->getProjectFiles('hca_unit_inspections_checklist', $id);
?>
		<div class="card">
			<div class="card-header">
