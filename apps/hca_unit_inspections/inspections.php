...
$access = ($User->checkAccess('hca_unit_inspections', 4)) ? true : false;
$access11 = ($User->checkAccess('hca_unit_inspections', 11)) ? true : false;
$access12 = ($User->checkAccess('hca_unit_inspections', 12)) ? true : false;
$access13 = ($User->checkAccess('hca_unit_inspections', 13)) ? true : false;
$access15 = ($User->checkAccess('hca_unit_inspections', 15)) ? true : false; // Reassign
if (!$access)
...
if (isset($_POST['reassign']))
{
	...
	{
		$DBLayer->update('hca_unit_inspections_checklist', $form_data, $checklist_id);
		...
		$mail_message[] = $URL->link('hca_unit_inspections_work_order', $checklist_id);
		...
		redirect($URL->link('hca_unit_inspections_inspections', $checklist_id).get_to_str(), $flash_message);
	}
}
...
$query = [
	'SELECT'	=> 'COUNT(c.id)',
	'FROM'		=> 'hca_unit_inspections_checklist as c',
	'JOINS'		=> [
		[
...
$Core->set_page_id('hca_unit_inspections_inspections', 'hca_unit_inspections');
...
?>
				...
				<div class="col-md-auto">
					...
					<a href="<?php echo $URL->link('hca_unit_inspections_inspections', 0) ?>" class="btn btn-sm btn-outline-secondary">Reset</a>
				</div>
			</div>
		</div>
	</form>
...
if (!empty($main_info)) 
{
	$uploader_info = $hca_unit_inspections_checklist_items = [];
	$query = array(
		'SELECT'	=> 'id, table_id',
		'FROM'		=> 'sm_uploader',
		'WHERE'		=> 'table_id IN ('.implode(',', $projects_ids).') AND table_name=\'hca_unit_inspections_checklist\''
	);
	$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
	while ($row = $DBLayer->fetch_assoc($result)) {
	...
	$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
	while ($row = $DBLayer->fetch_assoc($result)) {
		$hca_unit_inspections_checklist_items[] = $row;
	}
	...
	foreach($main_info as $cur_info)
	{
		$td1 = [];
		if ($access11)
			$Core->add_dropdown_item('<a href="'.$URL->link('hca_unit_inspections_checklist', $cur_info['id']).'"><i class="fas fa-edit"></i> CheckList</a>');

		if ($access12 && $cur_info['inspection_completed'] == 2 && $cur_info['num_problem'] > 0)
			$Core->add_dropdown_item('<a href="'.$URL->link('hca_unit_inspections_work_order', $cur_info['id']).'"><i class="fas fa-file-alt"></i> Work Order</a>');

		$Core->add_dropdown_item('<a href="'.$URL->link('hca_unit_inspections_files', $cur_info['id']).'"><i class="fas fa-file-upload"></i> Upload Files</a>');
		...
		if (in_array($cur_info['id'], $uploader_info))
			$td1[] = '<p><a href="'.$URL->link('hca_unit_inspections_files', $cur_info['id']).'" class="btn btn-sm btn-outline-success">Files</a></p>';


		$list_of_problems = [];
		$req_appendixb = false;
		if (!empty($hca_unit_inspections_checklist_items))
		{
			foreach($hca_unit_inspections_checklist_items as $checklist_items)
			{
				if ($cur_info['id'] == $checklist_items['checklist_id'])
				{
					$problem_names = $HcaUnitInspections->getItemProblems($checklist_items['problem_ids']);
...
/*
			...
			{
				if ($req_appendixb)
					$Core->add_dropdown_item('<a href="'.$URL->link('hca_unit_inspections_appendixb', $cur_info['id']).'"><i class="fas fa-file-pdf"></i> Add Appendix-B</a>');
			}
*/
		}
...
?>
			<tr id="row<?php echo $cur_info['id'] ?>" class="<?php echo ($cur_info['id'] == $id ? 'anchor' : '') ?>">
				...
				<td class="ta-center">
					...
					<p><?php echo format_date($cur_info['datetime_inspection_start'], 'n/j/Y') ?></p>
					<p><?php echo format_date($cur_info['datetime_inspection_start'], 'H:i') ?><?php echo $datetime_completion_end ?></p>
				</td>
				<td class="ta-center">
...
function reassignProject(id) {
	var csrf_token = "<?php echo generate_form_token($URL->link('hca_unit_inspections_ajax_reassign_project')) ?>";
	jQuery.ajax({
		url:	"<?php echo $URL->link('hca_unit_inspections_ajax_reassign_project') ?>",
		type:	"POST",
		dataType: "json",
		cache: false,
...
function getPropertyInfo(id) {
	var csrf_token = "<?php echo generate_form_token($URL->link('hca_unit_inspections_ajax_get_property_info')) ?>";
	jQuery.ajax({
		url:	"<?php echo $URL->link('hca_unit_inspections_ajax_get_property_info') ?>",
		type:	"POST",
		dataType: "json",
		cache: false,
		data: ({id:id,csrf_token:csrf_token}),
