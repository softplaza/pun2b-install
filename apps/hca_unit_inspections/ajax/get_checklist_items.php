...
if ($lid > 0)
{
	require SITE_ROOT.'apps/hca_unit_inspections/classes/HcaUnitInspections.php';
	...
	foreach($checklist_items as $cur_item)
	{
		//$equipment_item_name = $HcaUnitInspection->getEquipment($cur_item['equipment_id']).' - '.html_encode($cur_item['item_name']);
		$modal_body[] = '<option value="'.$cur_item['id'].'">'.$HcaUnitInspections->genItemName($cur_item).'</option>';
	}
	$modal_body[] = '</select>';
	$modal_body[] = '</div>';
