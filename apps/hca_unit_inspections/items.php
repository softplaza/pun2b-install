...
$access9 = ($User->checkAccess('hca_unit_inspections', 9)) ? true : false;
...
$locations = $HcaUnitInspections->locations;
...
if (isset($_POST['add']))
{
	...
	{
		// Create a new
		$new_id = $DBLayer->insert_values('hca_unit_inspections_items', $form_data);
...
else if (isset($_POST['update']))
{
	...
	if (empty($Core->errors))
	{
		$DBLayer->update('hca_unit_inspections_items', $form_data, $id);
...
{
	$DBLayer->delete('hca_unit_inspections_items', $id);
	...
	$FlashMessenger->add_info($flash_message);
	redirect($URL->link('hca_unit_inspections_items', 0), $flash_message);
}

if ($id > 0)
{
	$Core->set_page_title('item management');
	$Core->set_page_id('hca_unit_inspections_items', 'hca_unit_inspections');
	require SITE_ROOT.'header.php';

	$query = array(
		'SELECT'	=> 'i.*',
		'FROM'		=> 'hca_unit_inspections_items AS i',
		'WHERE'		=> 'i.id='.$id,
	);
	$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
...
<?php
	foreach($HcaUnitInspections->equipments as $key => $value)
	{
				...
				<div class="col-md-3">
					<div class="mb-3">
						...
						<select name="element_id" class="form-select form-select-sm" required>
							<option value="0">No element</option>
<?php
	foreach($HcaUnitInspections->elements as $key => $value)
	{
			...
			<div class="row">
				<div class="col-md-3">
					<div class="mb-3">
						...
						<input type="text" name="item_name" value="<?php echo html_encode($item_info['item_name']) ?>" class="form-control" id="fld_item_name">
					</div>
				</div>
				<div class="col-md-3">
					<div class="mb-3">
...
foreach($HcaUnitInspections->problems as $key => $value)
{
...
?>
			...
			<a href="<?php echo $URL->link('hca_unit_inspections_items', 0) ?>" class="btn btn-secondary text-white">Back</a>
		...
		<div class="accordion-body card-body">
			...
			<form method="post" accept-charset="utf-8" action="">
				...
				<div class="row">
					<div class="col-md-3">
						<div class="mb-3">
							<label class="form-label" for="fld_location_id">Locations</label>
							<select name="location_id" class="form-select form-select-sm" id="fld_location_id" required>
<?php
	foreach($locations as $location_id => $location_name)
		echo '<option value="'.$location_id.'">'.html_encode($location_name).'</option>';
?>
					...
					<div class="col-md-3">
						<div class="mb-3">
							<label class="form-label" for="fld_equipment_id">Equipments</label>
							<select name="equipment_id" class="form-select form-select-sm" id="fld_equipment_id">
								<option value="0">No equipment</option>
<?php
	foreach($HcaUnitInspections->equipments as $key => $value)
		echo '<option value="'.$key.'">'.html_encode($value).'</option>';
?>
					...
					<div class="col-md-3">
						<div class="mb-3">
							<label class="form-label" for="fld_element_id">Elements</label>
							<select name="element_id" class="form-select form-select-sm" id="fld_element_id" required>
								<option value="0">No equipment</option>
<?php
	foreach($HcaUnitInspections->elements as $key => $value)
		echo '<option value="'.$key.'">'.html_encode($value).'</option>';
?>
				...
				<div class="row">
					<div class="col-md-3">
						<div class="mb-3">
							<label class="form-label" for="fld_item_name">Display name</label>
							<input type="text" name="item_name" value="<?php echo isset($_POST['item_name']) ? html_encode($_POST['item_name']) : '' ?>" class="form-control" id="fld_item_name">
						</div>
					</div>
					<div class="col-md-3">
						<div class="mb-3">
...
?>
					<div class="form-check form-check-inline">
						<input type="hidden" name="problem[<?=$key?>]" value="0">
						<input class="form-check-input" id="fld_problem<?=$key?>" type="checkbox" name="problem[<?=$key?>]" value="1">
						<label class="form-check-label" for="fld_problem<?=$key?>"><?php echo $value ?></label>
					</div>
<?php
}
?>
				...
				<h6 class="card-title mb-0">Displaying of items</h6>
				<hr class="my-2">
				<div class="mb-2">
					<div class="form-check form-check-inline">
						<input type="hidden" name="display_in_checklist" value="0">
				...
				<div class="mb-2 hidden">
					<div class="form-check form-check-inline">
						<input type="hidden" name="req_appendixb" value="0">
						<input class="form-check-input" id="fld_req_appendixb" type="checkbox" name="req_appendixb" value="1" checked>
						<label class="form-check-label" for="fld_req_appendixb">Request Appendix-B form</label>
					</div>
				</div>
				<div class="mb-3">
...
<table class="table table-striped table-bordered table-hover">
	<thead>
		<tr>
			...
			<th class="hidden">Displayed in Summary report</th>
			<th>Position</th>
			<th></th>
		</tr>
	</thead>
	...
	foreach($locations as $location_id => $location_name)
	{
		foreach($main_info as $cur_info)
		{
			if ($location_id == $cur_info['location_id'])
			{
				...
				$problem_names = $HcaUnitInspections->getItemProblems($cur_info['problems']);
				$actions = ($access9) ? '<a href="'.$URL->link('hca_unit_inspections_items', $cur_info['id']).'" class="badge bg-primary text-white">Edit</a>' : '';
?>
		<tr class="<?php echo (($cur_info['display_in_checklist'] == 0) ? 'table-danger' : ($cur_info['summary_report'] == 0 ? 'table-warning' : '')) ?>">
			...
			<td class="fw-bold"><?php echo html_encode($HcaUnitInspections->genItemName($cur_info)) ?></td>
			<td class="fw-bold text-danger"><?php echo $problem_names ?></td>
			<td class="ta-center"><?php echo ($cur_info['display_in_checklist'] == 1 ? 'Yes' : '') ?></td>
			<td class="ta-center hidden"><?php echo ($cur_info['summary_report'] == 1 ? 'Yes' : '') ?></td>
