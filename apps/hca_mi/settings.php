...
else if (isset($_POST['add_new']))
{
	...
	if ($user_id > 0 && $new_access > 0)
	{
		$query = array(
			'UPDATE'	=> 'users',
			'SET'		=> 'hca_5840_access='.$new_access,
			'WHERE'		=> 'id='.$user_id
		);
		$DBLayer->query_build($query) or error(__FILE__, __LINE__);
		
		$flash_message = 'Access has been provided to #'.$user_id;
		$FlashMessenger->add_info($flash_message);
		redirect('', $flash_message);
	}
}
else if (isset($_POST['update']))
{
	...
	$hca_5840_mailing_fields = array('hca_5840_mailing_fields' => implode(',', array_keys($_POST['hca_5840_mailing_fields'])));
	
	$form = array_merge($form, $hca_5840_mailing_fields);
	
	$Config->update($form);
	
	if (isset($_POST['hca_5840_access']))
	{
		$projects_access = $_POST['hca_5840_access'];
		foreach ($projects_access as $id => $val) {
			$query = array(
				'UPDATE'	=> 'users',
				'SET'		=> 'hca_5840_access=\''.$DBLayer->escape($val).'\'',
				'WHERE'		=> 'id='.$id
			);
			$DBLayer->query_build($query) or error(__FILE__, __LINE__);
		}
...
<form method="post" accept-charset="utf-8" action="">
	...
	<div class="card">
		<div class="card-header">
			<h6 class="card-title mb-0">Create Access</h6>
		</div>
		<div class="card-body">
			<div class="row mb-3">
				<div class="col-md-12">
...
<?php
	...
	$query = array(
		'SELECT'	=> 'u.id, u.group_id, u.username, u.realname, u.email, u.hca_5840_access, g.g_id, g.g_title',
		'FROM'		=> 'groups AS g',
		'JOINS'		=> array(
			array(
				'INNER JOIN'	=> 'users AS u',
	...
	while ($fetch_assoc = $DBLayer->fetch_assoc($result)) {
		if ($fetch_assoc['hca_5840_access'] > 0)
			$assigned_users[] = $fetch_assoc;
		else
			$users_info[] = $fetch_assoc;
	}
?>
		<fieldset class="frm-group group">
			<div class="sf-set set">
				<div class="sf-box select">
					<label for="fld"><span>List of Employees</span><small>Select an employee from the dropdown list</small></label><br>
					<span class="fld-input"><select id="fld" name="user_id" required>
<?php
	$optgroup = 0;
...
?>
		...
		<fieldset class="frm-group frm-hdgroup group">
			<fieldset class="mf-set set">
<?php
$access_users = array(
	0 => 'Access denied',
	1 => 'View Projects',
					...
					<div class="mf-box">
						<div class="mf-field mf-field1 text">
							<span class="fld-input">
								<select id="fld" name="hca_5840_access[<?php echo $user['id'] ?>]">
<?php
		foreach ($access_users as $key => $value)
		{
			if ($user['hca_5840_access'] == $key)
...
foreach($hca_5840_mailing_fields_details as $key => $value) {
	if (in_array($key, $o_hca_5840_mailing_fields))
		$hca_5840_mailing_fields[] = ' <input type="checkbox" value="1" checked="checked" name="hca_5840_mailing_fields['.$key.']"> '.$value.' ';
	else
		$hca_5840_mailing_fields[] = ' <input type="checkbox" value="0" name="hca_5840_mailing_fields['.$key.']"> '.$value.' ';
}
?>
		<fieldset class="frm-group group">
			<div id="admin-alerts" class="ct-set warn-set">
				<div class="ct-box warn-box">
					<h6 class="ct-legend hn warn"><span>Details for Email</span></h6>
						<p><?php echo implode(' ', $hca_5840_mailing_fields) ?></p>
						<p>Check the boxes for the details you want to send</p>
				</div>
			</div>
			
			<div class="txt-set set">
				<div class="txt-box textarea">
					<label for="fld"><span>Mailing List</span><small>Insert emails separated by commas</small></label>
					<div class="txt-input"><span class="fld-input"><textarea id="fld" name="form[hca_5840_mailing_list]" rows="3" cols="55"><?php echo html_encode($Config->get('o_hca_5840_mailing_list')) ?></textarea></span></div>
				</div>
			</div>
			
			<div class="txt-set set">
				<div class="txt-box textarea">
					<label for="fld"><span>Locations</span><small>Insert locations separated by commas</small></label>
					<div class="txt-input"><span class="fld-input"><textarea id="fld" name="form[hca_5840_locations]" rows="5" cols="55"><?php echo html_encode($Config->get('o_hca_5840_locations')) ?></textarea></span></div>
				</div>
			</div>
		</fieldset>
