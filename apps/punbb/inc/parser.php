...
{
	require SITE_ROOT.'include/idna/idna_convert.class.php';
}
...
function preparse_bbcode($text, &$errors, $is_signature = false)
{
	...
	if ($Config->get('p_sig_bbcode') == '1' && $is_signature || $Config->get('p_message_bbcode') == '1' && !$is_signature)
	{
...
function preparse_tags($text, &$errors, $is_signature = false)
{
	...
	$tags_nested = array('quote' => $Config->get('o_quote_depth'), 'list' => 5, '*' => 5);
...
function split_text($text, $start, $end, &$errors, $retab = true)
{
	...
	if ($Config->get('o_indent_num_spaces') != 8 && $retab)
	{
		$spaces = str_repeat(' ', $Config->get('o_indent_num_spaces'));
...
function handle_img_tag($url, $is_signature = false, $alt = null)
{
	...
	if ($is_signature && $User->get('show_img_sig') != '0')
		$img_tag = '<img class="sigimage" src="'.$url.'" alt="'.forum_htmlencode($alt).'" />';
	else if (!$is_signature && $User->get('show_img') != '0')
...
{
	global $lang_common, $User, $Config;
	...
	if (($is_signature && $Config->get('p_sig_img_tag') == '1') || (!$is_signature && $Config->get('p_message_img_tag') == '1'))
	{
...
function parse_message($text, $hide_smilies)
{
	...
	if ($Config->get('p_message_bbcode') == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
		$text = do_bbcode($text);

	if ($Config->get('o_smilies') == '1' && $User->get('show_smilies') == '1' && $hide_smilies == '0')
...
function parse_signature($text)
{
	...
	if ($Config->get('p_sig_bbcode') == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
		$text = do_bbcode($text, true);

	if ($Config->get('o_smilies_sig') == '1' && $User->get('show_smilies') == '1')
		$text = do_smilies($text);
