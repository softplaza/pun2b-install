...
function hca_wom_IncludeCommon()
{
    ...
    if ($User->checkAccess('hca_wom'))
    {
        ...
        if ($User->checkAccess('hca_wom', 1))
            $SwiftMenu->addItem(['title' => 'Quick Task Entry', 'link' => $URL->link('hca_wom_wo_manager', 0), 'id' => 'hca_wom_wo_manager', 'parent_id' => 'hca_fs', 'level' => 1]);

        if ($User->checkAccess('hca_wom', 3))
            $SwiftMenu->addItem(['title' => 'Work Orders', 'link' => $URL->link('hca_wom_work_orders'), 'id' => 'hca_wom_work_orders', 'parent_id' => 'hca_fs', 'level' => 2]);
