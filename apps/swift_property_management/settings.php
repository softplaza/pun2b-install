...
$access = ($User->checkAccess('swift_property_management', 20)) ? true : false;
if (!$access)
	message($lang_common['No permission']);

$rules_to = 'swift_property_management';
$access_options = [
	// Pages
	1 => 'Properties',
	2 => 'Locations',
...
$query = array(
	'SELECT'	=> 'g.g_id, g.g_title, g_sm_property_mngr',
	'FROM'		=> 'groups AS g',
	'WHERE'		=> 'g.g_id!='.USER_GROUP_GUEST.' AND g.g_id!='.USER_GROUP_ADMIN,
	'ORDER BY'	=> 'g.g_id'
);
$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
$groups_info = $gids = array();
while ($row = $DBLayer->fetch_assoc($result)) {
	$groups_info[$row['g_id']] = $row;
	$gids[] = $row['g_id'];
}
...
else if (isset($_POST['save_settings']))
{
	...
	$form['sm_pm_unit_sizes'] = isset($form['sm_pm_unit_sizes']) ? preg_replace("/\s+/", '', $form['sm_pm_unit_sizes']) : array();
	
	$Config->update($form);

	$g_sm_property_mngr = isset($_POST['g_sm_property_mngr']) ? intval($_POST['g_sm_property_mngr']) : 0;
	
	if ($g_sm_property_mngr > 0 && !empty($gids))
	{
		// Resset group
		$query = array(
				'UPDATE'	=> 'groups',
				'SET'		=> 'g_sm_property_mngr=0',
				'WHERE'		=> 'g_id IN('.implode(',', $gids).')'
		);
		$DBLayer->query_build($query) or error(__FILE__, __LINE__);
		...
		$query = array(
				'UPDATE'	=> 'groups',
				'SET'		=> 'g_sm_property_mngr=1',
				'WHERE'		=> 'g_id='.$g_sm_property_mngr
		);
		$DBLayer->query_build($query) or error(__FILE__, __LINE__);
	}
...
$Core->set_page_id('sm_property_management_settings', 'sm_property_management');
		...
		<div class="card-body">
			...
			<div class="row" id="users_list">
				<div class="mb-3 col-md-3">
					<select name="user_id" class="form-select" id="fld_user_id">
<?php
	$optgroup = 0;
...
<form method="post" accept-charset="utf-8" action="">
	...
	<div class="card">
		...
		<div class="card-body">
			...
			<div class="mb-3 col-md-3">
				<label class="form-check-label fw-bold" for="fld_g_sm_property_mngr">Group of managers</label>
				<select id="fld_g_sm_property_mngr" class="form-select" name="g_sm_property_mngr">
<?php
echo "\t\t\t\t\t\t".'<option value="0" selected="selected" disabled>Select group</option>'."\n";
foreach ($groups_info as $cur_group)
{
	if ($cur_group['g_sm_property_mngr'] == 1)
...
?>
			...
			<div class="mb-3">
				<label for="fld_sm_pm_unit_sizes"><span>Unit Sizes</span><small>List all used unit sizes separated by commas.</small></label>
				<textarea id="fld_sm_pm_unit_sizes" name="form[sm_pm_unit_sizes]" class="form-control"><?php echo html_encode($Config->get('o_sm_pm_unit_sizes')) ?></textarea>
			</div>
			<div class="mb-3">
				<button type="submit" name="save_settings" class="btn btn-primary btn-sm">Update settings</button>
			</div>
