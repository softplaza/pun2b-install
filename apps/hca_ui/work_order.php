...
*/
	...
	if (empty($Core->errors))
	{
		...
		if (!empty($job_type))
		{
			foreach($job_type as $key => $value)
			{
				$DBLayer->update('hca_ui_checklist_items', ['job_type' => $value], $key);
			}
		}
...
if ($action != 'pdf')
	$SwiftMenu->addNavAction('<li><a class="dropdown-item" href="'.$URL->link('hca_ui_work_order', $id).'&action=pdf'.'" target="_blank"><i class="fa fa-file-pdf-o fa-1x" aria-hidden="true"></i> Print as PDF</a></li>');
else
...
<?php if ($main_info['inspection_completed'] == 1): ?>
	<div class="alert alert-danger" role="alert">
		...
		<a href="<?php echo $URL->link('hca_ui_checklist', $id) ?>" class="btn btn-secondary text-white mb-1">Go to Checklist</a>
	</div>
<?php else: ?>
...
foreach($HcaUnitInspection->getLocations() as $location_id => $location_name)
{
	if (!empty($checked_items))
	{
		...
		$files = $SwiftUploader->displayFiles('hca_ui_checklist'.$location_id, $id);
...
// Display files
$SwiftUploader->ajaxImages('hca_ui_checklist', $id);
...
<?php if ($appendixb) : ?>
			<a href="<?php echo $URL->link('hca_ui_appendixb', $id) ?>" class="btn btn-info text-white mb-1">Create Appendix-B</a>
<?php endif; ?>

<?php if ($User->checkAccess('hca_ui', 14)) : ?>
...
{
	var csrf_token = "<?php echo generate_form_token($URL->link('hca_ui_ajax_edit_work_order_item')) ?>";
	jQuery.ajax({
		url:	"<?php echo $URL->link('hca_ui_ajax_edit_work_order_item') ?>",
		type:	"POST",
		dataType: "json",
