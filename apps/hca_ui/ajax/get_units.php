...
if ($property_id > 0 && $unit_id > 0)
{
	...
	$query = [
		...
		'WHERE'		=> 'ch.unit_id='.$unit_id.' AND DATE(ch.date_inspected) > \''.$DBLayer->escape($DateTime->format('Y-m-d')).'\'',
	...
	if (!empty($checklist_dupes))
	{
		...
		foreach($checklist_dupes as $cur_info)
		{
			...
			$btn_actions[] = '<p><a href="'.$URL->link('hca_ui_checklist', $cur_info['id']).'" class="badge bg-primary text-white">Go to checklist</a></p>';
			$btn_actions[] = '</div>';
			$btn_actions[] = '</div>';
