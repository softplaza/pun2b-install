...
if (isset($_POST['upload_file']))
{
    ...
    $SwiftUploader->uploadFiles('sm_special_projects_records', $id);
...
else if (isset($_POST['send_files']))
{
	...
	if (empty($Core->errors))
	{
		...
		$url_param = [];
		$url_param[] = 'project=sm_special_projects_records';
		...
		//$SwiftMailer->isHTML();
		$SwiftMailer->send($emails, 'Special Project', $mail_message);
...
$query = array(
	...
	'FROM'		=> 'sm_uploader',
	'WHERE'		=> 'table_name=\'sm_special_projects_records\' AND table_id='.$id,
);
$result = $DBLayer->query_build($query) or error(__FILE__, __LINE__);
...
$Core->set_page_id('hca_sp_manage_files', 'hca_sp');
...
<?php $SwiftUploader->setForm();?>
	...
	<form method="post" accept-charset="utf-8" action="">
		<input type="hidden" name="csrf_token" value="<?php echo generate_form_token() ?>" />
<?php
	$SwiftUploader->getProjectFiles('sm_special_projects_records', $id);
?>
		<div class="card">
			<div class="card-header">
...
$mail_message = [];
//$mail_message[] = 'Hello,';
$mail_message[] = 'This is the Special Project.'."\n";
