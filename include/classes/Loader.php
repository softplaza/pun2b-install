...
// JS groups
define('SPM_JS_GROUP_SYSTEM', -100);
define('SPM_JS_GROUP_DEFAULT', 0);
define('SPM_JS_GROUP_COUNTER', 100);

// CSS groups
define('SPM_CSS_GROUP_SYSTEM', -100);
define('SPM_CSS_GROUP_DEFAULT', 0);
...
class Loader
{
	...
	public function addJS($data = NULL, $options = NULL, $local = false) {
		$this->add_js($data, $options, $local);
	}
	...
	public function add_js($data = NULL, $options = NULL)
	{
		...
		$default_options['data'] = swift_trim($data);
	...
	public function render_js()
	{
		...
		if (defined('SPM_DISABLE_ASYNC_JS_LOADER'))
		{
	...
	public function addCSS($data = NULL, $options = NULL, $local = false) {
		$this->add_css($data, $options, $local);
	}

	// Add CSS url to load
	public function add_css($data = NULL, $options = NULL, $local = false)
	{
		if (is_null($options) || !is_array($options))
		{
			$options = array();
		...
		foreach ($libs as $key => $lib)
		{
			if ($lib['type'] == 'inline')
			{
				if ($lib['noscript'] === true)
					$output .= swift_trim($this->check_conditional_comments($lib, '<noscript><style>'.$lib['data'].'</style></noscript>'))."\n";
				else
					$output .= swift_trim($this->check_conditional_comments($lib, '<style>'.$lib['data'].'</style>'))."\n";
				unset($libs[$key]);
				continue;
			}
			...
			{
				if ($lib['noscript'] === true)
					$output .= swift_trim($this->check_conditional_comments($lib, '<noscript><link rel="stylesheet" type="text/css" media="'.$lib['media'].'" href="'.$lib['data'].'" /></noscript>'))."\n";
				else
					$output .= swift_trim($this->check_conditional_comments($lib, '<link rel="stylesheet" type="text/css" media="'.$lib['media'].'" href="'.$lib['data'].'" />'))."\n";
				unset($libs[$key]);
				continue;
			}
		}
	...
	private function render_js_labjs()
	{
		...
		foreach ($libs as $key => $lib)
		{
			...
			if ($lib['type'] == 'inline')
			{
				if ($lib['group'] == SPM_JS_GROUP_SYSTEM)
				{
					$output_system .= '<script>'.$lib['data'].'</script>'."\n";
				}
				else if ($lib['group'] == SPM_JS_GROUP_COUNTER)
				{
	...
	/*
	// Autoloading css and js
	function autoload_ext($ext_id)
	{
		$js_path = getcwd().'/js/';
		$js_files = get_dir_files($js_path, 'js');
		$js_url = BASE_URL.'/extensions/'.$ext_id.'/js/';
		
		if (!empty($js_files)) {
			foreach($js_files as $js_file)
				$this->add_js($js_url.$js_file.'?'.time(), array('type' => 'url', 'async' => false, 'group' => 100 , 'weight' => 75));
		}
		
		$css_path = getcwd().'/css/';
		$css_files = get_dir_files($css_path, 'css');
		
		if (!empty($css_files)) {
			foreach($css_files as $css_file)
				$this->add_js($css_path.$css_file.'?'.time(), array('type' => 'url', 'async' => false, 'group' => 100 , 'weight' => 75));
		}
	}
*/
}
