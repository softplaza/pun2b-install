...
class SwiftPagination
{
	...
	// How many items to display on one page
	...
	function getNavi()
	{
		...
		if ($num_pages > 1)
		{
			...
			$paging = [];
			$paging[] = '<ul class="pagination">'.implode('', $pages).'</ul>';
			
			if ($total_items > 1)
				$paging[] = '<span class="page-link ms-2">Items: <span class="fw-bold">'.$items_per_page.'</span> of <span class="fw-bold">'.$total_items.'</span></span>';

			$output = (!empty($paging)) ? '<nav aria-label="Page navigation" class="nav-pagination">'."\n\t".implode("\n\t", $paging)."\n".'</nav>' : '';
		}
		else
			// Display num items only
			$output = ($total_items > 1) ? '<nav aria-label="Page navigation" class="nav-pagination">'."\n\t".'<span class="page-link ms-2">Items: <span class="fw-bold">'.$items_per_page.'</span> of <span class="fw-bold">'.$total_items.'</span></span>'."\n".'</nav>' : '';
