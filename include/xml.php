...
function xml_to_array($raw_xml)
{
	...
	foreach ($vals as $xml_elem)
	{
		...
		if (isset($xml_elem['value']) && swift_trim($xml_elem['value']) != '' && !array_key_exists('attributes', $xml_elem))
		{
			if ($x_type == 'open')
...
function validate_manifest($xml_array, $folder_name)
{
	...
	else
	{
		...
		if (isset($ext['minversion']) && version_compare(clean_version($Config->get('o_cur_version')), clean_version($ext['minversion']), '<'))
...
define('SPM_XML_FUNCTIONS_LOADED', 1);
