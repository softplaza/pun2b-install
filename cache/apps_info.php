<?php

define('CACHE_APPS_INFO_LOADED', 1);

$apps_info = array (
  'swift_events' => 
  array (
    'id' => 'swift_events',
    'title' => 'Swift Events',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/swift_events/',
    'path' => '../apps/swift_events/',
  ),
  'mpdf' => 
  array (
    'id' => 'mpdf',
    'title' => 'mPDF',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/mpdf/',
    'path' => '../apps/mpdf/',
  ),
  'sm_sent_emails' => 
  array (
    'id' => 'sm_sent_emails',
    'title' => 'Sent Emails',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/sm_sent_emails/',
    'path' => '../apps/sm_sent_emails/',
  ),
  'fancybox' => 
  array (
    'id' => 'fancybox',
    'title' => 'Fancybox',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/fancybox/',
    'path' => '../apps/fancybox/',
  ),
  'sm_backup_tables' => 
  array (
    'id' => 'sm_backup_tables',
    'title' => 'BackUp Tables',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/sm_backup_tables/',
    'path' => '../apps/sm_backup_tables/',
  ),
  'sm_errors_report' => 
  array (
    'id' => 'sm_errors_report',
    'title' => 'Errors Report',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/sm_errors_report/',
    'path' => '../apps/sm_errors_report/',
  ),
  'sm_user_actions' => 
  array (
    'id' => 'sm_user_actions',
    'title' => 'User Actions',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/sm_user_actions/',
    'path' => '../apps/sm_user_actions/',
  ),
  'hca_fs' => 
  array (
    'id' => 'hca_fs',
    'title' => 'HCA Facility Schedule',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_fs/',
    'path' => '../apps/hca_fs/',
  ),
  'swift_updater' => 
  array (
    'id' => 'swift_updater',
    'title' => 'Swift Updater',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/swift_updater/',
    'path' => '../apps/swift_updater/',
  ),
  'swift_property_management' => 
  array (
    'id' => 'swift_property_management',
    'title' => 'Property Management',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/swift_property_management/',
    'path' => '../apps/swift_property_management/',
  ),
  'sm_messenger' => 
  array (
    'id' => 'sm_messenger',
    'title' => 'Swift Messenger',
    'disabled' => '1',
    'url' => 'https://swiftmanager.localhost/dev/apps/sm_messenger/',
    'path' => '../apps/sm_messenger/',
  ),
  'hca_ui' => 
  array (
    'id' => 'hca_ui',
    'title' => 'HCA Plubming Inspections',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_ui/',
    'path' => '../apps/hca_ui/',
  ),
  'hca_repipe' => 
  array (
    'id' => 'hca_repipe',
    'title' => 'HCA Re-Pipe Projects',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_repipe/',
    'path' => '../apps/hca_repipe/',
  ),
  'hca_cc' => 
  array (
    'id' => 'hca_cc',
    'title' => 'HCA Compliance Calendar	',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_cc/',
    'path' => '../apps/hca_cc/',
  ),
  'fontawesome' => 
  array (
    'id' => 'fontawesome',
    'title' => 'Font Awesome Icons',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/fontawesome/',
    'path' => '../apps/fontawesome/',
  ),
  'swift_notify' => 
  array (
    'id' => 'swift_notify',
    'title' => 'Notification in Badges',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/swift_notify/',
    'path' => '../apps/swift_notify/',
  ),
  'swift_uploader' => 
  array (
    'id' => 'swift_uploader',
    'title' => 'Swift Uploader',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/swift_uploader/',
    'path' => '../apps/swift_uploader/',
  ),
  'punch_list_management' => 
  array (
    'id' => 'punch_list_management',
    'title' => 'Punch List Management',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/punch_list_management/',
    'path' => '../apps/punch_list_management/',
  ),
  'hca_vendors' => 
  array (
    'id' => 'hca_vendors',
    'title' => 'HCA Vendor Menagement',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_vendors/',
    'path' => '../apps/hca_vendors/',
  ),
  'hca_inventory' => 
  array (
    'id' => 'hca_inventory',
    'title' => 'HCA Inventory',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_inventory/',
    'path' => '../apps/hca_inventory/',
  ),
  'hca_sp' => 
  array (
    'id' => 'hca_sp',
    'title' => 'HCA Spacial Projects',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_sp/',
    'path' => '../apps/hca_sp/',
  ),
  'hca_mi' => 
  array (
    'id' => 'hca_mi',
    'title' => 'HCA Moisture Inspections',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_mi/',
    'path' => '../apps/hca_mi/',
  ),
  'hca_pc' => 
  array (
    'id' => 'hca_pc',
    'title' => 'HCA Pest Control Projects',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_pc/',
    'path' => '../apps/hca_pc/',
  ),
  'hca_trees' => 
  array (
    'id' => 'hca_trees',
    'title' => 'HCA Trees Projects',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_trees/',
    'path' => '../apps/hca_trees/',
  ),
  'hca_projects' => 
  array (
    'id' => 'hca_projects',
    'title' => 'HCA Projects',
    'disabled' => '1',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_projects/',
    'path' => '../apps/hca_projects/',
  ),
  'swift_calendar' => 
  array (
    'id' => 'swift_calendar',
    'title' => 'Calendar',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/swift_calendar/',
    'path' => '../apps/swift_calendar/',
  ),
  'hca_sb721' => 
  array (
    'id' => 'hca_sb721',
    'title' => 'HCA SB-721',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_sb721/',
    'path' => '../apps/hca_sb721/',
  ),
  'swift_messenger' => 
  array (
    'id' => 'swift_messenger',
    'title' => 'Swift Messenger',
    'disabled' => '1',
    'url' => 'https://swiftmanager.localhost/dev/apps/swift_messenger/',
    'path' => '../apps/swift_messenger/',
  ),
  'hca_vcr' => 
  array (
    'id' => 'hca_vcr',
    'title' => 'HCA VCR',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_vcr/',
    'path' => '../apps/hca_vcr/',
  ),
  'hca_unit_inspections' => 
  array (
    'id' => 'hca_unit_inspections',
    'title' => 'HCA Unit Inspections',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_unit_inspections/',
    'path' => '../apps/hca_unit_inspections/',
  ),
  'hca_hvac_inspections' => 
  array (
    'id' => 'hca_hvac_inspections',
    'title' => 'HCA HVAC Inspections',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_hvac_inspections/',
    'path' => '../apps/hca_hvac_inspections/',
  ),
  'hca_wom' => 
  array (
    'id' => 'hca_wom',
    'title' => 'Property Work Order Management',
    'disabled' => '0',
    'url' => 'https://swiftmanager.localhost/dev/apps/hca_wom/',
    'path' => '../apps/hca_wom/',
  ),
);

?>